...
class Message {
  ...
  public function getMessages($otherUser) {
    ...
    $data = "";
    $query = $this->connexion->prepare('UPDATE messages SET viewed = ? WHERE user_to = ? AND user_from = ?');
  ...
  public function getConvos() {
    ...
    foreach($convos as $username) {
      ...
      $row = $is_unread_query->fetch();
			...
			$style = ($row['viewed'] == 'no') ? "background-color: #DDEDFF" : "";
      ...
      $dots = (strlen($latest_message_details[2]) >= 12) ? "..." : "";
			...
			if($row['viewed'] === 'yes' && $row['user_from'] === $userLoggedIn && $row['user_to'] === $username) {

				$latest_message_details[2] .= " <b>✓</b>";
			}

			if ($row['viewed'] === 'no' && $row['user_from'] === $userLoggedIn && $row['user_to'] === $username) {

				$style = "";
				$latest_message_details[2] .= " <b>←</b>";
			}

			if($row['viewed'] === 'no' && $row['user_to'] === $userLoggedIn && $row['user_from'] === $username) {
  ...
  public function getConvosDropdown($data, $limit) {
    ...
    echo "<div class='new_message_dropdown' style='text-align:center;'><a href='messages.php?u=new'>New Message</a></div>";
    ...
    $set_viewed_query = $this->connexion->prepare('UPDATE messages SET opened = ? WHERE user_to = ?');
      ...
      if($count > $limit)
        ...
        $row = $is_unread_query->fetch();
  			...
  			if(strpos($latest_message_details[1], "http://") !== false) {
  				...
  				$return_string .=   "<div  class='dropdown_each_result'><a class='result_dropdown' href='messages.php?u=$username' ><div class='profile_pic_dropdown'><img  width='50px' src='" . $user_found_obj->getProfilePic() . "'></div><div style='".$style."' class='side_result_dropdown'><div class='names_result_dropdwon'>
              <span>".$user_found_obj->getFirstAndLastName()."</span></div><div class='time_frame_dropdown'><span>".$latest_message_details[2]."</span></div></div></a></div><hr>";
  			}

  			else {
  				$return_string .= "<div  class='dropdown_each_result'><a href='messages.php?u=$username'><div class='dropdown_each_result'><img  width='50px' src='" . $user_found_obj->getProfilePic() . "'></div><div style='".$style."' class='side_result_dropdown'><div <div class='names_result_dropdwon'>
            <span>".$user_found_obj->getFirstAndLastName()."</span></div><div class='time_frame_dropdown'><span>" . $latest_message_details[1] . "</span></div><div class='content_dropdown'><span>".$latest_message_details[2]."</span></div></a></div><hr>";
  			}
  		...
  		if($count > $limit)
  			...
  			$return_string .="<input type='hidden' class='noMoreDropdownData' value='true'><p style='text-align: center;'>No more message</p>";
  ...
  public function getUnreadNumber () {
    $userLoggedIn = $this->user_obj->getUsername();
    $query = $this->connexion->prepare('SELECT * FROM messages WHERE opened = ? AND user_to = ?');
